...
> 首次启动会在 exe 同级目录自动生成空数据文件，**个人数据只存在本机，不会上传到 GitHub**。
...
数据文件与 exe 同级目录（`dist/个人工作台/` 内），整个工具文件夹自包含可便携。

```
dist/个人工作台/
├── 个人工作台.exe          ← 启动器
├── _internal/              ← 依赖库（勿删）
├── workbench_data.json     ← 主数据（项目/待办/人员/周总结）
└── settings.json           ← 设置（字体大小）
```

> 换电脑直接拷贝整个 `个人工作台/` 文件夹即可，数据跟着走。
